"""
tools/analyse_training_phrases.py
─────────────────────────────────────────────────────────────────────────────
Deep NLU analysis of training phrases across intents.

Input:  CSV file with columns (utterance, intent)
        Produced by tools/phrase_extraction.py, or any CSV export from your
        NLU platform in the same format.

Output: Formatted Excel report with 6 analytical sheets.
...
6. Action Items          — prioritised remediation tasks (CRITICAL / HIGH / MEDIUM / LOW)

Platform-agnostic: works with any NLU platform that can export training
phrases in CSV format (utterance, intent columns).

Configuration (via .env):
  INPUT_CSV, OUTPUT_XLSX, OVERLAP_THRESHOLD, LOW_PHRASE_MIN

GCP integration (optional):
  After generating the report, push results to BigQuery:

    from gcp.bigquery_utils import push_phrase_analysis
    push_phrase_analysis(df_results)
─────────────────────────────────────────────────────────────────────────────
"""
...
# ── Config — all values from .env (see .env.example) ──────────────────────────
INPUT_CSV         = os.getenv("INPUT_CSV",  "output/training_phrases.csv")
OUTPUT_XLSX       = os.getenv("OUTPUT_XLSX", "output/training_phrase_analysis.xlsx")
...
def main():
    ...
    # upload_output(OUTPUT_XLSX)
    #
    # from gcp.bigquery_utils import push_phrase_analysis
    # import pandas as pd
    # push_phrase_analysis(pd.read_csv(INPUT_CSV))
